...
export CROSS_COMPILE=/opt/jetpack3.2/bin/aarch64-unknown-linux-gnu-
echo $CROSS_COMPILE
...
make -j 6 O=$TEGRA_KERNEL_OUT ARCH=$ARCH modules_prepare
make -j 6 O=$TEGRA_KERNEL_OUT ARCH=$ARCH zImage
make -j 6 O=$TEGRA_KERNEL_OUT ARCH=$ARCH dtbs
make -j 6 O=$TEGRA_KERNEL_OUT ARCH=$ARCH modules
make O=$TEGRA_KERNEL_OUT ARCH=$ARCH modules_install INSTALL_MOD_PATH=$TEGRA_BAS$
